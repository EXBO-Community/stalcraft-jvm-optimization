...
Оба бинарника пишут структурированные логи в каталог `logs/` рядом с exe:

- `logs/wrapper.log` — события враппера: запуск, детект железа, загрузка конфига, старт игрового процесса, exit code. Путь пользователя редактируется, сырые аргументы лаунчера и JVM-флаги не пишутся. Файл усечётся при превышении 2 MB.
- `logs/jvm.log` — unified logging JVM: GC-паузы, все STW safepoint'ы, JIT-компиляции, deoptimization, metaspace, code cache. 3 файла по 10 MB с ротацией. Позволяет диагностировать причины микрофризов.
