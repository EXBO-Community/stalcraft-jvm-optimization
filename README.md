...
**JVM (Java Virtual Machine)** — это среда выполнения, через которую работает [STALZONE](https://stalcraft.ru/).
...
> На системах с меньшим объёмом автосгенерированный профиль использует минимально безопасный heap,
...
`service.exe` перехватывает запуск игрового процесса `stalzone.exe` (лаунчер/EGS/VK Play) или `stalzonew.exe` (Steam) для:

- **Подбора оптимальной конфигурации JVM:** объём выделенных ресурсов, режим работы GC (Garbage Collector) и JIT-компиляции.
- **Запуска игры с выбранным JVM-профилем:** без изменения файлов игры и без внедрения в игровой процесс.
...
- **Права ОС:** права администратора в Windows (нужны только для установки перехвата)
- **ЦП:** 4 и более ядер
...
3. Скачайте [последнюю версию](../../releases/latest) и распакуйте `wrapper.zip` в папку `jvm_wrapper` — внутри должны оказаться `cli.exe`, `service.exe` и каталоги `examples/`, `langs/`.
...
После установки утилита автоматически создаст профиль конфигурации `v1.1.2/default.json`,
который будет располагаться в папке `jvm_wrapper/configs/v1.1.2/default.json`.
...
2. Выберите необходимый файл конфигурации и нажмите **Enter**. В папки версий можно заходить, пункт `< Back` возвращает на уровень выше.
3. Перезапустите игру, если она запущена.

> [!NOTE]
> По умолчанию активна конфигурация `v1.1.2/default.json`, однако она *не является* единственной опцией.
...
Те же операции доступны через команды:
...
#### Переопределение туннеля (экспериментально)

Пункт `Переопределение туннеля` позволяет независимо выбрать конкретный Roxy-узел для RU, EU, NA, SEA и NEA одновременно. `Выбор игры` находится внутри каждого региона и сбрасывает только его override. Список запрашивается только после открытия региона. При открытии группы утилита параллельно измеряет все ее узлы и отдельно показывает RTT от компьютера до туннеля, RTT от туннеля до backend и их сумму. Фоновых повторных раундов нет: новое измерение запускается только при повторном входе или через `Измерить снова`.

`Найти лучший сервер` проверяет все узлы вне постоянного списка исключений и выводит пять лучших. Результаты можно оценивать по полному пингу, RTT клиента, RTT сервера, стабильности или весу, который сообщает игровой backend. Узел с достигнутым лимитом подключений остается видимым, но выбрать его нельзя до свежего успешного измерения.

Выборы и настройки поиска хранятся в `jvm_wrapper/overrides.json`, история последних 20 измерений на узел — в `jvm_wrapper/cache/tunnel_stats.json`. Кэш хранится до 24 часов. `Regenerate Config` эти файлы не меняет. При старте игры `service.exe` только проверяет формат сохраненных адресов и добавляет отдельный JVM-флаг для каждого настроенного региона; сетевых измерений в пути запуска игры нет.

#### Примеры конфигурации

В репозитории лежит один пример — `examples/aikar.json`.
Он вдохновлён популярными Minecraft/Paper/Aikar-подходами к G1GC, но адаптирован под STALZONE и OpenJDK 9.
Это не точная копия серверных Aikar flags и не универсальная рекомендация.

Для `aikar.json` использовались как ориентиры [PaperMC Aikar flags](https://docs.papermc.io/paper/aikars-flags/) и предупреждение Prism Launcher о том, что [больше выделенной RAM не всегда означает выше производительность](https://prismlauncher.org/wiki/help-pages/java-settings/).

Чтобы использовать пример, перейдите в каталог репозитория [`/examples`](./examples/), скачайте `aikar.json` и положите его в `jvm_wrapper/configs/`.

Запустите утилиту, выберите `Select Config` в меню. Теперь в списке должен появиться ещё один профиль конфигурации. Выберите его, после чего перезапустите игру.

#### Кастомная конфигурация

Для создания своего собственного профиля конфигурации достаточно скопировать файл `configs/v1.1.2/default.json`,
...
> [!TIP]
> Если вы описали кастомную конфигурацию и хотите вернуться
> к рекомендованным настройкам — выберите в меню пункт `Regenerate Config` и нужную версию.
> Данное действие перезапишет `configs/<version>/default.json` и сделает его активным.
