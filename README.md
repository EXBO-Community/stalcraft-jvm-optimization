...
`service.exe` перехватывает запуск игрового процесса `stalzone.exe` (лаунчер) или `stalzonew.exe` (Steam) — а также прежние `stalcraft.exe` / `stalcraftw.exe`, пока игра не переименовала процесс — для:
